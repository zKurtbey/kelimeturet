<?php

$plybtn = document.querySelector('.btn');
$plybtn.addEventListener("click", function() {
  $plybtn->style->animation = "bb 1s ease-in-out forwards";
  start();
});
$lastword = [];
$a = 0;
$container = document.getElementById('container');
function start() {
  if(!$container){
    $container = document.createElement('div');
    $container->id = 'container';
    document.body->appendChild($container);
    createNewDiv();
    setupInputListeners($container);
    document.querySelector(".kel .hrf")->focus();
    $lastword = [];
  } else{
    createNewDiv();
    function setupInputListeners($container) {
      $divs = $container->querySelector('.kel');
      $inputs = $divs->querySelectorAll('input');
      $inputs->forEach(function($input) {
        $input->addEventListener("input", function() {
          moveToNextInput($input);
        });
      });
      $lastword = [];
    }
    document.querySelector(".kel .hrf")->focus();
  }
  $lastword = [];
}
function moveToNextInput($currentInput) {
  $inputValue = $currentInput->value;
  if ($inputValue->length === 1) {
    $nextInput = $currentInput->nextElementSibling;
    if ($nextInput) {
        $nextInput->focus();
        $currentInput->style = "border-bottom: 2px rgb(50, 120, 50) solid";
    } else {
      $currentInput->blur();
      $currentInput->style = "border-bottom: 2px rgb(50, 120, 50) solid";
      combineValues();
      }
  }
}
async function combineValues() {
  if(document.querySelector('.gk')){
    console.log($lastword);
  }
  $word = '';
  $divs = document.querySelector('.kel');
  $inputs = $divs->querySelectorAll('.kel input');
    $inputs->forEach(function($input) {
      $kck = $input->value->toLowerCase();
      $word += $kck;
      $input->maxLength = 1;
    });
  console.log($word);
  try {
    $response = await fetch('words.txt');
    $wordsText = await $response->text();
    $sozluk = $wordsText->split('\n');
    if ($sozluk->includes($word)) {
      if(document.querySelector(".gk")){
        $i = document.querySelectorAll('.gk')->length - 1;
        if(!$lastword->includes($word)){
          $gkson = document.querySelectorAll(".gk")[$i];
          console.log($lastword);
          if($gkson->querySelector("#sonharf")->value == document.querySelector(".kel #basharf")->value){
            $lastword->push($word);
            dogru();
        } else {
            harfuyus();
      }
        } else {
            oncedenyaz();
        }
      } else {
        $lastword->push($word);
        dogru();
      }
  } else {
      fail();
  }
} catch ($error) {
    console.error('Error fetching or reading words.txt:', $error);
  }
}
function createNewDiv() {
  $newDiv = document.createElement('div');
  $newDiv->setAttribute("class", "kel");
  $container->appendChild($newDiv);
  console.log($a);
  if ($a <= 5){
    $newInput = document.createElement('input');
      $newInput->type = 'text';
      $newDiv->appendChild($newInput);
      $newInput->classList->add("hrf");
      $newInput->setAttribute("oninput", "moveToNextInput(this)");
      $newInput->setAttribute("id", "basharf");
    $newInput->maxLength = 1;
      $newInput = document.createElement('input');
      $newInput->type = 'text';
      $newDiv->appendChild($newInput);
      $newInput->classList->add("hrf");
      $newInput->setAttribute("oninput", "moveToNextInput(this)");
    $newInput->maxLength = 1;
      $newInput = document.createElement('input');
      $newInput->type = 'text';
      $newDiv->appendChild($newInput);
      $newInput->classList->add("hrf");
      $newInput->setAttribute("oninput", "moveToNextInput(this)");
      document.querySelector(".kel .hrf")->focus();
      $newInput->setAttribute("id", "sonharf");
    $newInput->maxLength = 1;
    } else if ($a <= 10){
        $newInput = document.createElement('input');
        $newInput->type = 'text';
        $newDiv->appendChild($newInput);
        $newInput->classList->add("hrf");
        $newInput->setAttribute("oninput", "moveToNextInput(this)");
        $newInput->setAttribute("id", "basharf");
    $newInput->maxLength = 1;
        for ($i = 0; $i < 2; $i++) {
          $newInput = document.createElement('input');
          $newInput->type = 'text';
          $newDiv->appendChild($newInput);
          $newInput->classList->add("hrf");
          $newInput->setAttribute("oninput", "moveToNextInput(this)");
          $newInput->maxLength = 1;
        }
        $newInput = document.createElement('input');
        $newInput->type = 'text';
        $newDiv->appendChild($newInput);
        $newInput->classList->add("hrf");
        $newInput->setAttribute("oninput", "moveToNextInput(this)");
        document.querySelector(".kel .hrf")->focus();
        $newInput->setAttribute("id", "sonharf");
    $newInput->maxLength = 1;
    } else if ($a <= 15){
        $newInput = document.createElement('input');
          $newInput->type = 'text';
          $newDiv->appendChild($newInput);
          $newInput->classList->add("hrf");
          $newInput->setAttribute("oninput", "moveToNextInput(this)");
          $newInput->setAttribute("id", "basharf");
    $newInput->maxLength = 1;
          for ($i = 0; $i < 3; $i++) {
            $newInput = document.createElement('input');
            $newInput->type = 'text';
            $newDiv->appendChild($newInput);
            $newInput->classList->add("hrf");
            $newInput->setAttribute("oninput", "moveToNextInput(this)");
            $maxLength = 1;
          }
          $newInput = document.createElement('input');
          $newInput->type = 'text';
          $newDiv->appendChild($newInput);
          $newInput->classList->add("hrf");
          $newInput->setAttribute("oninput", "moveToNextInput(this)");
          document.querySelector(".kel .hrf")->focus();
          $newInput->setAttribute("id", "sonharf");
    $newInput->maxLength = 1;
    } else if ($a <= 20){
        $newInput = document.createElement('input');
          $newInput->type = 'text';
          $newDiv->appendChild($newInput);
          $newInput->classList->add("hrf");
          $newInput->setAttribute("oninput", "moveToNextInput(this)");
          $newInput->setAttribute("id", "basharf");
    $newInput->maxLength = 1;
          for ($i = 0; $i < 4; $i++) {
            $newInput = document.createElement('input');
            $newInput->type = 'text';
            $newDiv->appendChild($newInput);
            $newInput->classList->add("hrf");
            $newInput->setAttribute("oninput", "moveToNextInput(this)");
            $newInput->maxLength = 1;
          }
          $newInput = document.createElement('input');
          $newInput->type = 'text';
          $newDiv->appendChild($newInput);
          $newInput->classList->add("hrf");
          $newInput->setAttribute("oninput", "moveToNextInput(this)");
          document.querySelector(".kel .hrf")->focus();
          $newInput->setAttribute("id", "sonharf");
    $newInput->maxLength = 1;
        } else if ($a > 20){
        $newInput = document.createElement('input');
          $newInput->type = 'text';
          $newDiv->appendChild($newInput);
          $newInput->classList->add("hrf");
          $newInput->setAttribute("oninput", "moveToNextInput(this)");
          $newInput->setAttribute("id", "basharf");
    $newInput->maxLength = 1;
          for ($i = 0; $i < 4; $i++) {
            $newInput = document.createElement('input');
            $newInput->type = 'text';
            $newDiv->appendChild($newInput);
            $newInput->classList->add("hrf");
            $newInput->setAttribute("oninput", "moveToNextInput(this)");
            $newInput->maxLength = 1;
          }
          $newInput = document.createElement('input');
          $newInput->type = 'text';
          $newDiv->appendChild($newInput);
          $newInput->classList->add("hrf");
          $newInput->setAttribute("oninput", "moveToNextInput(this)");
          document.querySelector(".kel .hrf")->focus();
          $newInput->setAttribute("id", "sonharf");
    $newInput->maxLength = 1;
        }
  $a++;
}
function triggerBackgroundAnimation() {
  $yzd = 2;
  $snkyzd = parseFloat(document.body->style->backgroundPosition) || 0;
  $yniyzd =  $snkyzd + $yzd;
  document.body->style->backgroundPosition = "${yniyzd}% ${yniyzd}%";
}
function dogru(){
  $divs = document.querySelector('.kel');
  $divs->classList->remove("kel");
      triggerBackgroundAnimation();
      $bslk = document.querySelectorAll('.bslk');
      $bslk->forEach(function($bslk) {
        $topPercentage = .25;
        $lefPercentage = 1;
        $currentTopPercentage = parseFloat($bslk->style->marginTop) || 0;
        $newTopPercentage = $currentTopPercentage - $topPercentage;
        $bslk->style->marginTop = "${newTopPercentage}em";
        $currentLeftPercentage = parseFloat($bslk->style->marginLeft) || 0;
        $newLeftPercentage = $currentLeftPercentage - $lefPercentage;
        $bslk->style->marginLeft = "${newLeftPercentage}em";
      });
      $divs->classList->add("gk");
      $gk = document.querySelectorAll('.gk');
      $gk->forEach(function($gk) {
        $leftPercentage = 7.5;
        $currentLeftPercentage = parseFloat($gk->style->marginLeft) || 0;
        $newLeftPercentage = $currentLeftPercentage - $leftPercentage;
        $gk->style->marginLeft = "${newLeftPercentage}em";
        $topPercentage = 1.5;
        $currentTopPercentage = parseFloat($gk->style->marginTop) || 0;
        $newTopPercentage = $currentTopPercentage - $topPercentage;
        $gk->style->marginTop = "${newTopPercentage}em";
        $eskigiris = document.querySelectorAll('.gk input');
        $eskigiris->forEach(function($input) {
          $input->readOnly = true;
        });
      });
    setTimeout(function() {
      createNewDiv();
    }, 100);
}
function harfuyus(){
  document.getElementById("hatasebep")->innerHTML="Son harfler uyuşmuyor!";
  document.querySelector("#hata")->style="animation: hatavar 1.25s ease-in-out alternate; animation-iteration-count: 2";
  setTimeout(function() {
    document.querySelector("#hata")->style="";
  }, 2500);
  document.querySelector(".kel")->style="animation: scale .5s ease-in-out forwards";
  setTimeout(function() {
    setTimeout(function() {
      document.querySelectorAll('.kel input')->forEach(function($input) {
        $input->value="";
        $input->maxLength = 1;
    });
    }, 200);
    document.querySelector(".kel input")->focus();
    document.querySelector(".kel")->style="";
  }, 1000);
}
function oncedenyaz(){
  document.getElementById("hatasebep")->innerHTML="Bu kelimeyi zaten yazdınız!";
  document.querySelector("#hata")->style="animation: hatavar 1.25s ease-in-out alternate; animation-iteration-count: 2";
  setTimeout(function() {
    document.querySelector("#hata")->style="";
  }, 2500);
  document.querySelector(".kel")->style="animation: scale .5s ease-in-out forwards";
  setTimeout(function() {
    setTimeout(function() {
      document.querySelectorAll('.kel input')->forEach(function($input) {
        $input->value="";
        $input->maxLength = 1;
      });
      document.querySelector(".kel input")->focus();
    }, 200);
    document.querySelector(".kel")->style="";
  }, 1000);
}
function fail(){
$lastword = [];
  $a = 0;
    $hrf = document.querySelectorAll('.kel .hrf');
    $hrf->forEach(function($hrf) {
      $hrf->style="animation: fail .5s ease-in-out alternate; animation-iteration-count: 5; border-bottom: 2px rgb(200, 70, 70) solid; color: rgb(200, 70, 70);";
    });
  setTimeout(function() {
    document.body->style->backgroundPosition="0% 0%";
    $gk = document.querySelectorAll('.gk');
    if($gk){
      $gk->forEach(function($gk) {
        $gk->style="animation: scale .4s ease-in forwards";
      });
    }
    $divs = document.querySelector('.kel');
    $divs->style="animation: scale .5s ease-in forwards; animation-delay: .25s";
    document.querySelector("#klm")->style="top: 0; left: 5%;";
    document.querySelector("#trt")->style="top: 9%; left: 12%;";
    setTimeout(function() {
      $gk->forEach(function($gk) {
        $gk->classList->remove("gk");
        $gk->classList->add("yok");
        document.querySelector('.yok')->remove();
      });
      setTimeout(function() {
        $divs->remove();
      }, 500);
      $plybtn->style->animation="gg 1s ease-in-out forwards";
    }, 1000);
  }, 2000);
}
?>
